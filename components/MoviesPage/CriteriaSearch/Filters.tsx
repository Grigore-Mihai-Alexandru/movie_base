...
const Filters: React.FC <props> = ({option, setOption, genres}) => {
    ...
    const {with_genres, with_runtime_gte, with_runtime_lte, release_date_gte, release_date_lte} = router.query
    
    const [collapse, setCollapse] = useState<boolean>(false)
    const [date, setDate] = useState<string>("")
    const [selectedGenres, setSelectedGenres] = useState<string | string[]>(with_genres !== undefined ?with_genres:"")
    const [runtime,setRuntime] = useState<string>("")
    const [filters, setFilters] = useState({})
    
    useEffect(()=>{
        if(selectedGenres !== "")
        setFilters(prev =>  { return {...prev, selectedGenres}})
        if(date !== "")
        setFilters(prev =>  { return {...prev, date}})
        if(runtime !== "")
        setFilters(prev =>  { return {...prev, runtime}})
            ...
            {collapse &&
                <div className='border-t-2 '>
                    <div className='p-4'>
                        <p>Date:</p>
                        <hr/>
                        <DateInput release_date_gte={release_date_gte} release_date_lte={release_date_lte} date={date} setDate={setDate} />
                    </div>
                    ...
                    <div className="p-4">
                        <p>Runtime:</p>
                        <hr/>
                        <Runtime runtime={runtime} setRuntime={setRuntime} with_runtime_gte={with_runtime_gte} with_runtime_lte={with_runtime_lte} />
                    </div>
                </div>
            }
        </div>
